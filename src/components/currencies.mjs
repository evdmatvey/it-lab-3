...
import { UpdateCurrencyComponent } from './update-currency.mjs';

export class CurrenciesComponent {
  layout = `
    <li class="settings__item-item--default settings__currency-item--default">
      RUB - опорная валюта
    </li>
  `;

  constructor(selector, onCurrencyUpdate) {
  ...
  addCurrenciesToList() {
    ...
    currencies.forEach((currency) => {
      this.element.insertAdjacentHTML(
        'beforeend',
        this.getCurrencyLayout(currency.name, currency.rates, currency.id),
      );
    });
  }

  initHandlers() {
    const currencies = this.element.querySelectorAll('li.settings__currency-item');

    currencies.forEach((currency) => {
      const currencyId = currency.dataset.id;
      const deleteButton = currency.querySelector('.currency-item__controls-delete');
      const updateButton = currency.querySelector('.currency-item__controls-update');

      const deleteHandler = () => this.deleteCurrency(currencyId);
      const updateHandler = () =>
        new UpdateCurrencyComponent('.update__currency-form', currencyId, () => {
  ...
  deleteCurrency(id) {
    CurrencyController.delete(id);
    this.render();
  }

  getCurrencyLayout(name, rates, id) {
    return `
      <li class="settings__item-item settings__currency-item" data-id=${id}>
        ${name}
        <div class="settings-item__item-rate currency__item-rate">${rates.at(-1).rate ?? 0}</div>
        <div class="settings-item__item-controls currency__item-controls">
          <button class="settings-item__item-controls-update currency-item__controls-update" popovertarget="update-currency" popovertargetaction="show">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
          ...
          <button class="settings-item__item-controls-delete currency-item__controls-delete">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
