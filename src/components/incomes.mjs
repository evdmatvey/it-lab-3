...
import { UpdateIncomeComponent } from './update-income.mjs';
...
export class IncomesComponent {
  ...
  render(timeGap) {
    ...
    this.addIncomesToList(timeGap);
    this.initListeners();
  }

  initListeners() {
    const incomes = this.element.querySelectorAll('.incomes__table-item');

    incomes.forEach((income) => {
      const incomeId = income.dataset.id;
      const deleteHandler = () => this.deleteHandler(incomeId);
      const updateHandler = () => {
        new UpdateIncomeComponent('.update__income-form', incomeId, () => {
          this.onUpdate();
          this.render('Текущая неделя');
        });
      };

      const deleteButton = income.querySelector('.transactions-item__info-delete');
      const updateButton = income.querySelector('.transactions-item__info-update');
  ...
  deleteHandler(id) {
    IncomeController.delete(+id);
    this.render('Текущая неделя');
  }

  calculateTotalElement(timeGap) {
    const { start, end } = getPeriodDates(timeGap);
    const { incomes } = Budget.filter(start, end);
    const moneys = incomes.map((i) => ({
      currency: i.currency,
      value: i.value,
    }));
  ...
  addIncomesToList(timeGap) {
    const { start, end } = getPeriodDates(timeGap);
    const { incomes } = Budget.filter(start, end);
    incomes.forEach((income) => {
      this.element.insertAdjacentHTML('beforeend', this.getIncomeLayout(income));
    });
  }

  getIncomeLayout(income) {
    const { id, name, type, value, currency } = income;
    const formattedAmount = formatCurrency(value, currency);

    return `
      <div class="incomes__table-item transactions__table-item" data-id=${id}>
        <div class="incomes__item-info transactions__item-info">
          <h3 class="incomes-item__info-title transactions-item__info-title">
            ${name}
            <div class="transactions-item__info-controls">
              <button class="transactions-item__info-update" popovertarget="update-income"
          ...
          <p class="incomes-item__info-category transactions-item__info-category">
            ${type}
          </p>
        </div>
        <div class="incomes__item-price transactions__item-price">${formattedAmount}</div>
      </div>
    `;
  }
}
