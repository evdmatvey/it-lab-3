...
import { UpdateIncomeTypeComponent } from './update-income-type.mjs';

export class IncomeTypesComponent {
  constructor(selector, onIncomeTypeUpdate) {
    this.onIncomeTypeUpdate = onIncomeTypeUpdate;
  ...
  render() {
    ...
    this.addIncomeTypesToList();
  }

  addIncomeTypesToList() {
    const incomeTypes = IncomeTypeController.get();
    incomeTypes.forEach((incomeType) => {
      this.element.insertAdjacentHTML(
        'beforeend',
        this.getIncomeTypeLayout(incomeType.name, incomeType.id),
      );
    });
  }

  initHandlers() {
    const incomeTypes = this.element.querySelectorAll('li.settings__incomes-item');

    incomeTypes.forEach((incomeType) => {
      const incomeTypeId = incomeType.dataset.id;
      const deleteButton = incomeType.querySelector('.incomes-item__controls-delete');
      const updateButton = incomeType.querySelector('.incomes-item__controls-update');

      const deleteHandler = () => this.deleteIncomeType(incomeTypeId);
      const updateHandler = () =>
        new UpdateIncomeTypeComponent('.update__incomes-form', incomeTypeId, () => {
          this.onIncomeTypeUpdate();
          this.render();
        });
  ...
  deleteIncomeType(id) {
    IncomeTypeController.delete(id);
    this.render();
  }

  getIncomeTypeLayout(name, id) {
    return `
      <li class="settings__item-item settings__incomes-item" data-id=${id}>
        ${name}
        <div class="settings-item__item-controls incomes__item-controls">
          <button class="settings-item__item-controls-update incomes-item__controls-update" popovertarget="update-incomes-type" popovertargetaction="show">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
          ...
          <button class="settings-item__item-controls-delete incomes-item__controls-delete">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
